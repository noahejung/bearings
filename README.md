...
- `GET /api/map?address=<str>` -> real map geometry for the neighbourhood around one address: real building footprints (each with its own real PLUTO year built and open Class C HPD count, for the building click card) and street centrelines, plus GTFS subway/PATH alignments and stations, each carrying its real served routes. Feeds the navigable map's local overlay (`web/src/components/MapView.tsx`). **Every layer is a bbox slice of a build-time-baked Parquet file** -- `sources/buildings.py`, `sources/streets.py` and (since 2026-09-07) `sources/gtfs.py` -- so this endpoint makes no live external call at all. It used to also return five per-H3-cell metrics for a k=3 disk, three of which were live Socrata queries per request; nothing in the frontend read them and the identical five metrics are already served, baked and citywide, by `GET /api/cells`. Removed 2026-09-07 -- see `src/bearings/mapgeo.py`'s own module docstring.
...
src/bearings/
  ...
  mapgeo.py              # real map geometry for one address: real building mass
                          # + street hairlines + subway lines/stations (with
                          # served routes), all bbox-sliced from baked Parquet
